...
function buildSlots(blocks, sessionMin) {
    const slots = [];
...
function makePlan(req) {
    ...
    const scoredAssignments = [...pendingAssignments].sort((a, b) => {
        const scoreA = (0, urgencyScore_1.computeUrgencyScore)(a, req.now);
        const scoreB = (0, urgencyScore_1.computeUrgencyScore)(b, req.now);
